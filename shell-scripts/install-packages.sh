# General utilities

echo "------- Installing packages from aptitude --------\n\n "------- 
sudo apt-get install terminator code git deluge curl gparted python3-pip python3-dev python3-venv okular vlc filezilla vokoscreen tree gimp htop mediainfo ffmpeg p7zip-full usb-creator-gtk net-tools openssh-server x11vnc gnome-tweaks \

sudo snap install postman
# Reminder for virtual environment initialization
#echo "\nvirtualenv --python=/usr/bin/python3.5 py-env"
echo "Don't forget to python3 -m venv py-env on ~/Documents/Codigos to initialize the Python virtual environment!

# Echoing programs that aren't available in aptitude and i should not forget to install
echo "\nDon't forget to install: Postman, Telegram, URSim, Todoist https://github.com/KryDos/todoist-linux/releases \n"

# Utilites for OpenCV in Ubuntu 16.04
#sudo apt-get install libjpeg8-dev libtiff5-dev libjasper-dev libpng12-dev libavcodec-dev libavformat-dev libswscale-dev libv4l-dev libxvidcore-dev libx264-dev libgtk-3-dev libatlas-base-dev gfortran python2.7-dev python3.5-dev
